...
`;
   ...
   */
  public async visualizeCode(code: string, language: string): Promise<any> {
    if (!this.model) {
      throw new Error("Gemini API key is not configured.");
    }
...
`;
    ...
    try {
      ...
      console.error("LLMService Error:", error);
      throw new Error(
        "Failed to generate visualization trace from Gemini API. Output was likely not valid JSON.",
      );
    }
  }
   ...
   */
  public async getLoopIterations(
    code: string,
    language: string,
    triggerLine: string
  ): Promise<any> {
...
`;
    ...
    try {
      ...
      throw new Error(
        "Failed to generate loop iterations from Gemini API. Output was likely not valid JSON."
      );
    }
  }
}
